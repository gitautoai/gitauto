...
- LOGGERS: Do not delete existing logger statements unless they are outdated. Every `continue`, `break`, and `return` (including early returns) inside a function MUST be preceded by a `logger.info(...)` (or `logger.warning(...)` etc.) call explaining what happened and why execution is stopping or skipping. Silent exits make debugging impossible.
