...
- Adapt the template to match the repo's package manager and test framework (e.g., yarn instead of npm, vitest instead of jest, unittest instead of pytest, gradle instead of maven). Do NOT adapt to E2E frameworks (see below)
...
- Instead, create a workflow for a test framework that CAN produce coverage
...
- If the repo has no test framework installed, add it as a devDependency (e.g., npm install --save-dev jest, pip install pytest-cov) as part of the workflow setup
