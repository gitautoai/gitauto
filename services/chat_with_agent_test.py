...
def mock_tools_to_call():
    ...
    with patch("services.chat_with_agent.tools_to_call", {
...
def test_chat_with_agent_function_correction(
    ...
    mock_print.assert_called_once()
    assert "Warning: Redirecting call from 'apply_diff_to_file' to 'replace_remote_file_content'" in mock_print.call_args[0][0]
...
def test_chat_with_agent_different_modes(
    ...
    # Test each mode
    for mode in ["comment", "commit", "explore", "get", "search"]:
        result = chat_with_agent(
            messages=messages,
            trigger="issue_label",
            base_args=base_args,
        ...
        if mode == "comment":
            assert kwargs["tools"] == chat_with_agent.TOOLS_TO_UPDATE_COMMENT
        elif mode == "commit":
            assert kwargs["tools"] == chat_with_agent.TOOLS_TO_COMMIT_CHANGES
        elif mode == "explore":
            assert kwargs["tools"] == chat_with_agent.TOOLS_TO_EXPLORE_REPO
        elif mode == "get":
            assert kwargs["tools"] == chat_with_agent.TOOLS_TO_GET_FILE
        elif mode == "search":
            assert kwargs["tools"] == chat_with_agent.TOOLS_TO_SEARCH_GOOGLE