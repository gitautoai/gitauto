...
async def test_run_eslint_fix_sets_npm_cache_env_on_lambda(base_args):
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                "services.eslint.run_eslint_fix.get_efs_dir",
                return_value="/mnt/efs/test",
            ):
                with patch("services.eslint.run_eslint_fix.extract_dependencies"):
...
async def test_run_eslint_fix_returns_fixed_content(base_args):
    ...
    with patch(
        ...
        with patch(
            "services.eslint.run_eslint_fix.get_efs_dir", return_value="/mnt/efs/test"
        ):
            with patch("services.eslint.run_eslint_fix.extract_dependencies"):
                with patch("services.eslint.run_eslint_fix.os.makedirs"):
...
async def test_run_eslint_fix_creates_directories(base_args):
    ...
    with patch(
        ...
        with patch(
            ...
            with patch("services.eslint.run_eslint_fix.extract_dependencies"):
                with patch(
                    "services.eslint.run_eslint_fix.os.makedirs"
                ) as mock_makedirs:
                    with patch("builtins.open", mock_open(read_data=file_content)):
                        with patch(
                            "services.eslint.run_eslint_fix.subprocess.run"
                ...
                with patch("services.eslint.run_eslint_fix.os.makedirs"):
                    with patch("builtins.open", mock_open(read_data="formatted")):
                        with patch(
                            ...
                            assert "env" in call_kwargs
                            assert (
                                call_kwargs["env"]["ESLINT_USE_FLAT_CONFIG"] == "false"
                            )
