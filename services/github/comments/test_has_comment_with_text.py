...
def mock_get_all_comments():
    ...
    with patch("services.github.comments.has_comment_with_text.get_all_comments") as mock:
        yield mock
...
def test_has_comment_with_text_missing_login_field(base_args, mock_get_all_comments):
    ...
    mock_comments = [
        {
            "user": {},
            "body": "Comment without login field"
        },
        {
            "user": {"login": GITHUB_APP_USER_NAME},
            "body": "This comment has the target text"
...
def test_has_comment_with_text_get_all_comments_exception(base_args, mock_get_all_comments):
    ...
    # Assert
    assert result is False  # Should return default value due to handle_exceptions decorator
