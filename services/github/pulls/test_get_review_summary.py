...
MODULE = "services.github.pulls.get_review_summary"


def test_get_review_summary_success():
    ...
    with patch(f"{MODULE}.requests.get") as mock_get, patch(
        ...
        result = get_review_summary("owner", "repo", 1, 100, "token")
...
def test_get_review_summary_http_error():
    with patch(f"{MODULE}.requests.get") as mock_get, patch(
        f"{MODULE}.create_headers"
    ) as mock_headers:
        mock_response = MagicMock()
        ...
        result = get_review_summary("owner", "repo", 1, 999, "token")

        assert result is None


def test_get_review_summary_network_error():
    with patch(f"{MODULE}.requests.get") as mock_get, patch(
        f"{MODULE}.create_headers"
    ) as mock_headers:
