...
def test_is_pull_request_open_returns_true_when_pr_is_open(mock_graphql_client, sample_params):
...
def test_is_pull_request_open_returns_false_when_pr_is_closed(mock_graphql_client, sample_params):
...
def test_is_pull_request_open_returns_false_when_pr_is_merged(mock_graphql_client, sample_params):
...
def test_is_pull_request_open_returns_false_when_repository_not_found(mock_graphql_client, sample_params):
...
def test_is_pull_request_open_returns_false_when_pull_request_not_found(mock_graphql_client, sample_params):
...
def test_is_pull_request_open_returns_false_when_empty_response(mock_graphql_client, sample_params):
...
def test_is_pull_request_open_calls_graphql_with_correct_parameters(mock_graphql_client, sample_params):
...
def test_is_pull_request_open_creates_graphql_client_with_token(sample_params):
    ...
    with patch("services.github.pulls.is_pull_request_open.get_graphql_client") as mock_get_client:
        mock_client = MagicMock()
