...
def test_update_reference_success(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    ...
    expected_url = (
        f"{GITHUB_API_URL}/repos/test-owner/test-repo/git/refs/heads/feature-branch"
    )
    ...
    mock_requests_patch.assert_called_once_with(
        url=expected_url, json=expected_data, headers=expected_headers, timeout=TIMEOUT
    )
    mock_response.raise_for_status.assert_called_once()
    assert result is None


def test_update_reference_with_different_branch(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_with_different_owner_repo(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_with_different_token(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_with_long_commit_sha(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
...
def test_update_reference_with_short_commit_sha(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
...
def test_update_reference_http_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
...
def test_update_reference_connection_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_timeout_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_request_exception_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_uses_correct_timeout(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
...
def test_update_reference_uses_patch_method(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
...
def test_update_reference_sends_json_data(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
...
def test_update_reference_calls_raise_for_status(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
...
def test_update_reference_with_special_characters_in_branch(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_with_unicode_characters_in_branch(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    ...
    expected_url = (
        f"{GITHUB_API_URL}/repos/test-owner/test-repo/git/refs/heads/feature-测试"
    )
    mock_requests_patch.assert_called_once()
    call_args = mock_requests_patch.call_args
    assert call_args[1]["url"] == expected_url
...
def test_update_reference_with_empty_commit_sha(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
...
def test_update_reference_with_numeric_commit_sha(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
...
def test_update_reference_rate_limit_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
...
def test_update_reference_unauthorized_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
...
def test_update_reference_server_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
    mock_response = MagicMock()
...
def test_update_reference_json_decode_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_attribute_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_key_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_type_error_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_generic_exception_returns_false(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_with_various_commit_sha_formats(
    sample_base_args, mock_requests_patch, mock_create_headers, commit_sha
):
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
    mock_requests_patch.return_value = mock_response
...
def test_update_reference_with_various_branch_names(
    sample_base_args, mock_requests_patch, mock_create_headers, branch_name
):
    modified_args = sample_base_args.copy()
    ...
    expected_url = (
        f"{GITHUB_API_URL}/repos/test-owner/test-repo/git/refs/heads/{branch_name}"
    )
    mock_requests_patch.assert_called_once()
    call_args = mock_requests_patch.call_args
    assert call_args[1]["url"] == expected_url
...
def test_update_reference_various_http_errors_return_false(
    sample_base_args,
    mock_requests_patch,
    mock_create_headers,
    status_code,
...
def test_update_reference_extracts_correct_values_from_base_args(
    sample_base_args, mock_requests_patch, mock_create_headers
):
...
def test_update_reference_preserves_request_structure(
    sample_base_args, mock_requests_patch, mock_create_headers
):
