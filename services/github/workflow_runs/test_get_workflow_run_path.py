...
def test_get_workflow_run_path_success(mock_successful_response, mock_headers):
    ...
    # Act
    with patch("services.github.workflow_runs.get_workflow_run_path.get") as mock_get, \
         patch("services.github.workflow_runs.get_workflow_run_path.create_headers") as mock_create_headers:
    ...
    assert mock_get.call_args[1]["url"] == f"https://api.github.com/repos/{OWNER}/{REPO}/actions/runs/{run_id}"
...
def test_get_workflow_run_path_url_construction():
    ...
    with patch("services.github.workflow_runs.get_workflow_run_path.get") as mock_get, \
         patch("services.github.workflow_runs.get_workflow_run_path.create_headers") as mock_create_headers, \
         patch("services.github.workflow_runs.get_workflow_run_path.GITHUB_API_URL", "https://api.github.test"):
        mock_create_headers.return_value = {"Authorization": f"Bearer {token}"}
        mock_get.return_value = mock_response
    ...
    # Assert
    mock_get.assert_called_once()
    assert mock_get.call_args[1]["url"] == f"https://api.github.test/repos/{owner}/{repo}/actions/runs/{run_id}"
...
def test_get_workflow_run_path_timeout_parameter():
    ...
    with patch("services.github.workflow_runs.get_workflow_run_path.get") as mock_get, \
         patch("services.github.workflow_runs.get_workflow_run_path.create_headers") as mock_create_headers, \
         patch("services.github.workflow_runs.get_workflow_run_path.TIMEOUT", 60):
        mock_create_headers.return_value = {"Authorization": f"Bearer {TOKEN}"}
        mock_get.return_value = mock_response
        get_workflow_run_path(OWNER, REPO, run_id, TOKEN)
