...
class TestEnsureTsconfigTest:
    def _make_base_args(self) -> BaseArgs:
        return cast(
            BaseArgs,
            {
                "owner": "test-owner",
    ...
    @patch("services.node.ensure_tsconfig_test.replace_remote_file_content")
    @patch("services.node.ensure_tsconfig_test.get_raw_content")
    @patch("services.node.ensure_tsconfig_test.get_file_tree")
    def test_creates_file_when_no_variants_exist(
        self, mock_tree: MagicMock, mock_get_raw: MagicMock, mock_replace: MagicMock
    ):
        mock_tree.return_value = [{"type": "blob", "path": "tsconfig.json"}]
        mock_get_raw.return_value = None
        mock_replace.return_value = "Success"

        result = ensure_tsconfig_test(
