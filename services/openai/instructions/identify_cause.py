...
Given information such as a pull request title, body, changes, workflow file content, and check run error log, identify the cause of the Check Run failure and write a plan to fix the error in the language that is used in the input. For example, if the input is mainly in Japanese, the plan should be in Japanese.
