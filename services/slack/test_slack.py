...
def test_slack_url_set():
    ...
    with patch("services.slack.slack.URL", test_url), \
        ...
        # Verify the function returned None (due to the decorator)
        assert result is None
...
def test_slack_with_request_exception():
    ...
    with patch("services.slack.slack.URL", test_url), \
        ...
        # Call the function - should not raise an exception due to the decorator
        result = slack(test_message)
...
def test_slack_with_http_error():
    ...
    with patch("services.slack.slack.URL", test_url), \
        ...
        # Create a mock HTTPError with the mock response
