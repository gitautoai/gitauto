...
import pytest
from config import FREE_TIER_REQUEST_AMOUNT, PRODUCT_ID_FOR_FREE, PRODUCT_ID_FOR_STANDARD
...
def mock_stripe_product():
    ...
    with patch("services.stripe.get_base_request_limit.stripe.Product.retrieve") as mock:
        yield mock
...
def free_tier_product_response():
    ...
    return {
        ...
        "name": "Free Tier",
        "metadata": {
            "request_count": str(FREE_TIER_REQUEST_AMOUNT)
        }
    }


def test_get_base_request_limit_returns_correct_value(mock_stripe_product, sample_product_response):
...
def test_get_base_request_limit_with_free_tier_product(mock_stripe_product, free_tier_product_response):
...
def test_get_base_request_limit_handles_invalid_request_count_format(mock_stripe_product):
...
@pytest.mark.parametrize("product_id,expected_request_count", [
    ("prod_basic", "50"),
    ("prod_premium", "2000"),
    ("prod_enterprise", "10000"),
])
def test_get_base_request_limit_with_various_products(mock_stripe_product, product_id, expected_request_count):
