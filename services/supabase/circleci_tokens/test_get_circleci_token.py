...
class TestGetCircleciToken:
    ...
    def test_handles_negative_owner_id(self, mock_supabase_with_token):
        """Test that function works with negative owner_id."""
        # Arrange
        mock_supabase, mock_table, expected_token_data = mock_supabase_with_token
        owner_id = -1
    ...
    def test_returns_first_token_when_multiple_exist(self):
        ...
        mock_response = Mock()
        mock_response.data = [token_data_1, token_data_2]
        ...
        with patch("services.supabase.circleci_tokens.get_circleci_token.supabase") as mock_supabase:
            ...
            # Assert
            assert result == token_data_1  # Should return the first token
            mock_table.limit.assert_called_once_with(1)  # Limit should ensure only one result

    def test_handle_exceptions_decorator_applied(self):
        """Test that the handle_exceptions decorator is properly applied."""
        # Arrange
        with patch("services.supabase.circleci_tokens.get_circleci_token.supabase") as mock_supabase:
            # Simulate various types of exceptions that could occur
            exceptions_to_test = [
    ...
    def test_query_parameters_are_correct(self, mock_supabase_with_token):
        """Test that the correct query parameters are used."""
        # Arrange
        mock_supabase, mock_table, _ = mock_supabase_with_token
        owner_id = 54321
