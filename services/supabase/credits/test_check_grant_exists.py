...
def test_check_grant_exists_returns_true_when_grants_exist(mock_supabase, mock_query_chain, mock_query_result):
...
def test_check_grant_exists_returns_true_when_single_grant_exists(mock_supabase, mock_query_chain, mock_query_result):
...
def test_check_grant_exists_returns_false_when_no_grants_exist(mock_supabase, mock_query_chain, mock_query_result):
...
def test_check_grant_exists_with_various_owner_ids(mock_supabase, mock_query_chain, mock_query_result, owner_id):
...
def test_check_grant_exists_handles_result_data_access_exception(mock_supabase, mock_query_chain):
...
def test_check_grant_exists_verifies_correct_table_name(mock_supabase, mock_query_chain, mock_query_result):
...
def test_check_grant_exists_verifies_correct_select_field(mock_supabase, mock_query_chain, mock_query_result):
...
def test_check_grant_exists_verifies_correct_owner_id_filter(mock_supabase, mock_query_chain, mock_query_result):
...
def test_check_grant_exists_verifies_correct_transaction_type_filter(mock_supabase, mock_query_chain, mock_query_result):
...
def test_check_grant_exists_handles_attribute_error_on_result(mock_supabase, mock_query_chain):
...
@pytest.mark.parametrize("data_content,expected_result", [
    ([{"id": 1}], True),
    ([{"id": 1}, {"id": 2}], True),
    ([{"id": 1}, {"id": 2}, {"id": 3}], True),
    ([], False),
])
def test_check_grant_exists_with_different_data_sizes(mock_supabase, mock_query_chain, data_content, expected_result):
...
def test_check_grant_exists_handles_key_error_exception(mock_supabase, mock_query_chain):
...
def test_check_grant_exists_handles_value_error_exception(mock_supabase, mock_query_chain):
...
def test_check_grant_exists_return_type_is_boolean(mock_supabase, mock_query_chain, mock_query_result):
...
def test_check_grant_exists_return_type_is_boolean_when_false(mock_supabase, mock_query_chain, mock_query_result):
...
def test_check_grant_exists_with_complex_data_structure(mock_supabase, mock_query_chain):
    """Test that check_grant_exists works with complex data structures in result."""
    # Arrange
    owner_id = 151515
    mock_query_result = MagicMock()
    mock_query_result.data = [
        {"id": 1, "owner_id": owner_id, "transaction_type": "grant", "amount": 10},
        {"id": 2, "owner_id": owner_id, "transaction_type": "grant", "amount": 5}
    ]
    mock_query_chain['eq2'].execute.return_value = mock_query_result
