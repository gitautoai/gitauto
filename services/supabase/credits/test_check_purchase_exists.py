...
def test_returns_true_when_purchase_exists(mock_supabase):
    ...
    mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.limit.return_value.execute.return_value = (
        mock_result
    )

    assert check_purchase_exists(owner_id=123) is True
...
def test_returns_false_when_no_purchase(mock_supabase):
    ...
    assert check_purchase_exists(owner_id=456) is False
...
def test_queries_credits_table_with_purchase_type(mock_supabase):
    ...
    mock_eq2.limit.return_value.execute.return_value = mock_result

    check_purchase_exists(owner_id=789)

    mock_supabase.table.assert_called_with("credits")
    mock_table.select.assert_called_with("id")
    mock_select.eq.assert_called_with("owner_id", 789)
    mock_eq1.eq.assert_called_with("transaction_type", "purchase")
