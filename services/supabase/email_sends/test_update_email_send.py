...
def test_update_email_send_sets_resend_email_id(mock_supabase):
    ...
    mock_eq3.execute.return_value = MagicMock()
    ...
    mock_update.eq.assert_called_once_with("platform", "github")
    mock_eq1.eq.assert_called_once_with("owner_id", 12345)
    mock_eq2.eq.assert_called_once_with("email_type", "uninstall")
    mock_eq3.execute.assert_called_once()
