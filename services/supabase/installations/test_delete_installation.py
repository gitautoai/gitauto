...
def mock_datetime_now():
    ...
    with patch("services.supabase.installations.delete_installation.datetime") as mock_dt:
...
def test_delete_installation_with_zero_installation_id(mock_supabase, mock_datetime_now):
...
def test_delete_installation_with_negative_installation_id(mock_supabase, mock_datetime_now):
...
def test_delete_installation_with_large_installation_id(mock_supabase, mock_datetime_now):
...
def test_delete_installation_with_special_characters_in_user_name(mock_supabase, mock_datetime_now):
...
def test_delete_installation_supabase_exception_handling(mock_supabase, mock_datetime_now):
...
@pytest.mark.parametrize("installation_id,user_id,user_name", [
    (1, 100, "user1"),
...
def test_delete_installation_decorator_behavior():
    ...
    with patch("services.supabase.installations.delete_installation.supabase") as mock_supabase:
        mock_supabase.table.return_value.update.return_value.eq.return_value.execute.return_value = "some_value"
        
        result = delete_installation(12345, 67890, "test-user")
        assert result is None  # Should return None due to decorator default_return_value
