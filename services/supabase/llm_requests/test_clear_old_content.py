...
def test_clear_old_content_single_batch(mock_datetime, mock_supabase):
    ...
    mock_in.execute.return_value = mock_update_result
...
@patch("services.supabase.llm_requests.clear_old_content.supabase")
@patch("services.supabase.llm_requests.clear_old_content.datetime")
def test_clear_old_content_multiple_batches(mock_datetime, mock_supabase):
    fixed_now = datetime(2025, 12, 1, 12, 0, 0)
    mock_datetime.now.return_value = fixed_now
    ...
    mock_select_result2 = Mock()
    mock_select_result2.data = batch2_ids
...
@patch("services.supabase.llm_requests.clear_old_content.supabase")
@patch("services.supabase.llm_requests.clear_old_content.datetime")
def test_clear_old_content_no_records(mock_datetime, mock_supabase):
    fixed_now = datetime(2025, 12, 1, 12, 0, 0)
    mock_datetime.now.return_value = fixed_now
    ...
    mock_lt_select.neq.return_value = mock_neq
    mock_neq.limit.return_value = mock_limit
    mock_limit.execute.return_value = mock_select_result
