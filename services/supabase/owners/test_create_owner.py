...
def test_create_owner_success_with_required_parameters_only(mock_supabase):
    ...
    # Execute
    create_owner(
        owner_id=789, owner_name="minimal-owner", user_id=101, user_name="minimal-user"
    )
...
def test_create_owner_supabase_exception_handled(mock_supabase):
    ...
    result = create_owner(
        owner_id=666, owner_name="error-owner", user_id=777, user_name="error-user"
    )
...
def test_create_owner_with_zero_ids(mock_supabase):
    ...
    # Execute
    create_owner(
        owner_id=0, owner_name="zero-id-owner", user_id=0, user_name="zero-id-user"
    )
...
def test_create_owner_key_error_handled(mock_supabase):
    ...
    mock_supabase.table.return_value.insert.return_value.execute.side_effect = KeyError(
        "Key error"
    )
