...
def mock_supabase():
    ...
    with patch("services.supabase.owners.update_stripe_customer_id.supabase") as mock:
        ...
        mock_eq.execute = MagicMock(return_value=mock_execute_result)
...
def test_update_stripe_customer_id_success(mock_supabase):
    ...
    mock_supabase.table().update().eq.assert_called_once_with("platform", "github")
    mock_supabase.table().update().eq().eq.assert_called_with("owner_id", 123456)
    mock_supabase.table().update().eq().eq().execute.assert_called_once()
...
def test_update_stripe_customer_id_with_empty_string(mock_supabase):
    ...
    # Execute
    result = update_stripe_customer_id(
        platform="github", owner_id=789, stripe_customer_id="", updated_by="1:user"
    )
    ...
    mock_supabase.table().update().eq.assert_called_once_with("platform", "github")
    mock_supabase.table().update().eq().eq.assert_called_with("owner_id", 789)
    mock_supabase.table().update().eq().eq().execute.assert_called_once()
    assert result is None
...
def test_update_stripe_customer_id_with_long_customer_id(mock_supabase):
    ...
    mock_supabase.table().update().eq().eq.assert_called_with("owner_id", 456)
    assert result is None
...
def test_update_stripe_customer_id_with_special_characters(mock_supabase):
    ...
    mock_supabase.table().update().eq().eq.assert_called_with("owner_id", 999)
    assert result is None
...
def test_update_stripe_customer_id_with_unicode_characters(mock_supabase):
    ...
    mock_supabase.table().update().eq().eq.assert_called_with("owner_id", 111)
    assert result is None
...
def test_update_stripe_customer_id_with_zero_owner_id(mock_supabase):
    ...
    result = update_stripe_customer_id(
        platform="github",
        owner_id=0,
        stripe_customer_id="cus_zero123",
        updated_by="1:user",
    )

    # Assert
    mock_supabase.table().update().eq().eq.assert_called_with("owner_id", 0)
    assert result is None
...
def test_update_stripe_customer_id_with_negative_owner_id(mock_supabase):
    ...
    result = update_stripe_customer_id(
        platform="github",
        owner_id=-1,
        stripe_customer_id="cus_negative123",
        updated_by="1:user",
    )

    # Assert
    mock_supabase.table().update().eq().eq.assert_called_with("owner_id", -1)
    assert result is None
...
def test_update_stripe_customer_id_with_large_owner_id(mock_supabase):
    ...
    # Assert
    mock_supabase.table().update().eq().eq.assert_called_with(
        "owner_id", large_owner_id
    )
    assert result is None
...
def test_update_stripe_customer_id_table_method_called_correctly(mock_supabase):
    ...
    update_stripe_customer_id(
        platform="github",
        owner_id=111,
        stripe_customer_id="cus_table123",
        updated_by="1:user",
    )
...
def test_update_stripe_customer_id_method_chaining(mock_supabase):
    ...
    update_stripe_customer_id(
        platform="github",
        owner_id=888,
        stripe_customer_id="cus_chain123",
        updated_by="1:user",
    )
    ...
    mock_supabase.table().update().eq().eq.assert_called_once()
    mock_supabase.table().update().eq().eq().execute.assert_called_once()
...
def test_update_stripe_customer_id_attribute_error_handled(mock_supabase):
    ...
    mock_supabase.table.return_value.update.return_value.eq.return_value.eq.return_value.execute.side_effect = AttributeError(
        "Attribute error"
    )
    result = update_stripe_customer_id(
        platform="github",
...
def test_update_stripe_customer_id_key_error_handled(mock_supabase):
    ...
    mock_supabase.table.return_value.update.return_value.eq.return_value.eq.return_value.execute.side_effect = KeyError(
        "Key error"
    )
    result = update_stripe_customer_id(
        platform="github",
...
def test_update_stripe_customer_id_type_error_handled(mock_supabase):
    ...
    mock_supabase.table.return_value.update.return_value.eq.return_value.eq.return_value.execute.side_effect = TypeError(
        "Type error"
    )
    result = update_stripe_customer_id(
        platform="github",
...
def test_update_stripe_customer_id_parametrized(
    ...
    mock_supabase.table().update().eq().eq.assert_called_with("owner_id", owner_id)
    assert result is None
...
def test_update_stripe_customer_id_function_signature():
    ...
    # Assert expected parameters are present (kwargs-only including platform)
    expected_params = ["platform", "owner_id", "stripe_customer_id", "updated_by"]
