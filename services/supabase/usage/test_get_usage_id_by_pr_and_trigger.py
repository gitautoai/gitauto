...
def _build_chain(execute_data):
    ...
    mock_eq4.limit.return_value = mock_limit
...
def test_get_usage_id_by_pr_and_trigger_returns_id_when_row_exists():
    ...
    chain[2].eq.assert_called_once_with(column="installation_id", value=80763246)
    chain[3].eq.assert_called_once_with(column="repo_id", value=279688027)
    chain[4].eq.assert_called_once_with(column="pr_number", value=1159)
    chain[5].eq.assert_called_once_with(column="trigger", value="schedule")
    chain[6].limit.assert_called_once_with(size=1)
