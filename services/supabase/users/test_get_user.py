...
def test_get_user_returns_user_when_found(sample_user_data):
    with patch("services.supabase.users.get_user.supabase") as mock_supabase:
        mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.execute.return_value = _mock_response(
            [sample_user_data]
        )

        result = get_user(platform="github", user_id=123)
...
def test_get_user_returns_none_when_not_found():
    with patch("services.supabase.users.get_user.supabase") as mock_supabase:
        ...
        result = get_user(platform="github", user_id=999)
...
def test_get_user_with_various_user_ids(user_id):
    with patch("services.supabase.users.get_user.supabase") as mock_supabase:
        ...
        result = get_user(platform="github", user_id=user_id)
...
def test_get_user_integration():
    result = get_user(platform="github", user_id=46782860)
    assert result is not None
    assert result["user_id"] == 46782860

    result = get_user(platform="github", user_id=999999999)
    assert result is None
