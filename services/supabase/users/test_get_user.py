from unittest.mock import patch, MagicMock
from typing import Any

import pytest

from services.supabase.users.get_user import get_user


@pytest.fixture
def mock_supabase():
    """Fixture to provide a mocked supabase client."""
    with patch("services.supabase.users.get_user.supabase") as mock:
        yield mock


@pytest.fixture
def sample_user_data():
    """Fixture providing sample user data."""
    return {
        "user_id": 123,
        "user_name": "test_user",
        "email": "test@example.com",
        "created_at": "2023-01-01T00:00:00Z",
        "updated_at": "2023-01-01T00:00:00Z"
    }


@pytest.fixture
def mock_supabase_response_with_user(sample_user_data):
    """Fixture providing a mock supabase response with user data."""
    return ((None, [sample_user_data]), None)


@pytest.fixture
def mock_supabase_response_empty():
    """Fixture providing a mock supabase response with no user data."""
    return ((None, []), None)


def test_get_user_returns_user_when_found(mock_supabase, mock_supabase_response_with_user, sample_user_data):
    """Test that get_user returns user data when user is found."""
    # Setup mock chain
    mock_table = MagicMock()
    mock_select = MagicMock()
    mock_eq = MagicMock()
    
    mock_supabase.table.return_value = mock_table
    mock_table.select.return_value = mock_select
    mock_select.eq.return_value = mock_eq
    mock_eq.execute.return_value = mock_supabase_response_with_user
    
    # Call function
    result = get_user(user_id=123)
    
    # Verify result
    assert result == sample_user_data
    
    # Verify supabase calls
    mock_supabase.table.assert_called_once_with(table_name="users")
    mock_table.select.assert_called_once_with("*")
    mock_select.eq.assert_called_once_with(column="user_id", value=123)
    mock_eq.execute.assert_called_once()


def test_get_user_returns_none_when_not_found(mock_supabase, mock_supabase_response_empty):
    """Test that get_user returns None when user is not found."""
    # Setup mock chain
    mock_table = MagicMock()
    mock_select = MagicMock()
    mock_eq = MagicMock()
    
    mock_supabase.table.return_value = mock_table
    mock_table.select.return_value = mock_select
    mock_select.eq.return_value = mock_eq
    mock_eq.execute.return_value = mock_supabase_response_empty
    
    # Call function
    result = get_user(user_id=999)
    
    # Verify result
    assert result is None
    
    # Verify supabase calls
    mock_supabase.table.assert_called_once_with(table_name="users")
    mock_table.select.assert_called_once_with("*")
    mock_select.eq.assert_called_once_with(column="user_id", value=999)
    mock_eq.execute.assert_called_once()


def test_get_user_returns_first_user_when_multiple_found(mock_supabase, sample_user_data):
    """Test that get_user returns the first user when multiple users are found."""
    # Create multiple user data
    second_user_data = {
        "user_id": 124,
        "user_name": "second_user",
        "email": "second@example.com",
        "created_at": "2023-01-02T00:00:00Z",
        "updated_at": "2023-01-02T00:00:00Z"
    }
    mock_response = ((None, [sample_user_data, second_user_data]), None)
    
    # Setup mock chain
    mock_table = MagicMock()
    mock_select = MagicMock()
    mock_eq = MagicMock()
    
    mock_supabase.table.return_value = mock_table
    mock_table.select.return_value = mock_select
    mock_select.eq.return_value = mock_eq
    mock_eq.execute.return_value = mock_response
    
    # Call function
    result = get_user(user_id=123)
    
    # Verify result - should return first user
    assert result == sample_user_data
    assert result["user_id"] == 123
    assert result["user_name"] == "test_user"


def test_get_user_handles_supabase_exception(mock_supabase):
    """Test that get_user handles exceptions gracefully and returns None."""
    # Setup mock to raise exception
    mock_supabase.table.side_effect = Exception("Database connection error")
    
    # Call function
    result = get_user(user_id=123)
    
    # Verify result - should return None due to exception handling
    assert result is None


def test_get_user_handles_malformed_response(mock_supabase):
    """Test that get_user handles malformed response gracefully."""
    # Setup mock with malformed response
    mock_response = (None, None)  # Invalid response structure
    
    mock_table = MagicMock()
    mock_select = MagicMock()
    mock_eq = MagicMock()
    
    mock_supabase.table.return_value = mock_table
    mock_table.select.return_value = mock_select
    mock_select.eq.return_value = mock_eq
    mock_eq.execute.return_value = mock_response
    
    # Call function
    result = get_user(user_id=123)
    
    # Verify result - should return None due to exception handling
    assert result is None


@pytest.mark.parametrize("user_id", [
    1,
    999999,
    0,
    -1,
])
def test_get_user_with_various_user_ids(mock_supabase, mock_supabase_response_empty, user_id):
    """Test that get_user handles various user ID formats correctly."""
    # Setup mock chain
    mock_table = MagicMock()
    mock_select = MagicMock()
    mock_eq = MagicMock()
    
    mock_supabase.table.return_value = mock_table
    mock_table.select.return_value = mock_select
    mock_select.eq.return_value = mock_eq
    mock_eq.execute.return_value = mock_supabase_response_empty
    
    # Call function
    result = get_user(user_id=user_id)
    
    # Verify result
    assert result is None
    
    # Verify the correct user_id was passed
    mock_select.eq.assert_called_once_with(column="user_id", value=user_id)


def test_get_user_type_annotation_compliance(sample_user_data):
    """Test that get_user returns the correct type as per annotation."""
    with patch("services.supabase.users.get_user.supabase") as mock_supabase:
        mock_response = (None, [sample_user_data])
        mock_supabase.table.return_value.select.return_value.eq.return_value.execute.return_value = mock_response
        
        result = get_user(user_id=123)
        
        # Verify type compliance
        assert isinstance(result, dict)
        assert all(isinstance(key, str) for key in result.keys())
