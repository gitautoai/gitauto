...
async def test_skips_when_comment_author_not_gitauto(base_payload):
    """Test that handler exits early when comment author is not GitAuto."""
    from services.webhook.pr_checkbox_handler import handle_pr_checkbox_trigger
...
@pytest.mark.asyncio
async def test_skips_when_no_files_selected(base_payload):
    """Test that handler exits early when no files are selected."""
    from services.webhook.pr_checkbox_handler import handle_pr_checkbox_trigger

    with patch("services.webhook.pr_checkbox_handler.extract_selected_files") as mock_extract, \
...
@pytest.mark.asyncio
async def test_access_denied_creates_comment_and_exits(base_payload):
    """Test access denied path."""
    from services.webhook.pr_checkbox_handler import handle_pr_checkbox_trigger
    ...
    with patch("services.webhook.pr_checkbox_handler.get_installation_access_token") as mock_token, \
        ...
        second_call = mock_slack.call_args_list[1]
        assert second_call[0][0] == "User has no credits remaining"


@pytest.mark.asyncio
async def test_timeout_handling_breaks_loop(base_payload):
    ...
    with patch("services.webhook.pr_checkbox_handler.get_installation_access_token") as mock_token, \
        ...
        mock_timeout.return_value = (True, 890.5)
        ...
        timeout_call = None
        for call in mock_update_comment.call_args_list:
...
@pytest.mark.asyncio
async def test_pr_closed_during_execution(base_payload):
    """Test PR closed during execution."""
    from services.webhook.pr_checkbox_handler import handle_pr_checkbox_trigger
    ...
    with patch("services.webhook.pr_checkbox_handler.get_installation_access_token") as mock_token, \
        ...
        update_calls = mock_update_comment.call_args_list
        pr_closed_call = None
...
@pytest.mark.asyncio
async def test_branch_deleted_during_execution(base_payload):
    """Test branch deleted during execution."""
    from services.webhook.pr_checkbox_handler import handle_pr_checkbox_trigger
...
@pytest.mark.asyncio
async def test_retry_logic_not_explored_and_committed(base_payload):
    """Test retry logic when not explored but committed."""
    from services.webhook.pr_checkbox_handler import handle_pr_checkbox_trigger
...
@pytest.mark.asyncio
async def test_retry_logic_explored_but_not_committed(base_payload):
    """Test retry logic when explored but not committed."""
    from services.webhook.pr_checkbox_handler import handle_pr_checkbox_trigger
...
@pytest.mark.asyncio
async def test_retry_logic_resets_on_success(base_payload):
    """Test retry logic resets counter on success."""
    from services.webhook.pr_checkbox_handler import handle_pr_checkbox_trigger
...
@pytest.mark.asyncio
async def test_credit_billing_with_depleted_credits_sends_email(base_payload):
    """Test credit depletion email notification."""
    from services.webhook.pr_checkbox_handler import handle_pr_checkbox_trigger
    ...
    with patch("services.webhook.pr_checkbox_handler.get_installation_access_token") as mock_token, \
        ...
        mock_insert_credit.assert_called_once_with(
            owner_id=11111,
            transaction_type="usage",
            usage_id=888
        )
        mock_get_owner.assert_called_once_with(owner_id=11111)
        mock_get_user.assert_called_once_with(user_id=22222)
...
@pytest.mark.asyncio
async def test_credit_billing_with_positive_balance_no_email(base_payload):
...
@pytest.mark.asyncio
async def test_subscription_billing_no_credit_insert(base_payload):
...
@pytest.mark.asyncio
async def test_token_accumulation_across_multiple_iterations(base_payload):
    ...
    with patch("services.webhook.pr_checkbox_handler.get_installation_access_token") as mock_token, \
        ...
        usage_call = mock_update_usage.call_args.kwargs
...
@pytest.mark.asyncio
async def test_complete_successful_flow(base_payload):
    ...
    with patch("services.webhook.pr_checkbox_handler.get_installation_access_token") as mock_token, \
        ...
        mock_empty_commit.assert_called_once()
        mock_update_usage.assert_called_once()

        final_update_call = mock_update_comment.call_args_list[-1]
