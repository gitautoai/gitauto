...
def test_get_default_structured_rules_success(
    mock_requests_get, sample_structured_rules
):
    ...
    mock_requests_get.assert_called_once_with(
        url="https://gitauto.ai/api/structured-rules-default", timeout=120
    )
...
def test_get_default_structured_rules_url_construction():
    ...
    with patch(
        "services.website.get_default_structured_rules.requests.get"
    ) as mock_get, patch(
        "services.website.get_default_structured_rules.BASE_URL",
        ...
        mock_get.assert_called_once_with(
            url="https://custom-domain.com/api/structured-rules-default", timeout=120
        )
...
def test_get_default_structured_rules_timeout_parameter():
    ...
    with patch(
        "services.website.get_default_structured_rules.requests.get"
    ) as mock_get, patch("services.website.get_default_structured_rules.TIMEOUT", 60):
