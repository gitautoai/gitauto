...
# Test without the decorator to see if there are any exceptions
def remove_pytest_sections_no_decorator(error_log: str):
    if not error_log:
        return error_log
    ...
    for line in lines:
        ...
        # stop skipping. Be very conservative here.
        if skip and line.strip():
            # Only stop skipping for lines that clearly don't belong to pytest sections
            # This handles cases like warnings summary followed by regular log content
...
try:
    result = remove_pytest_sections_no_decorator(test_input)
...
# Test with the decorated version
from utils.logs.remove_pytest_sections import remove_pytest_sections

result_decorated = remove_pytest_sections(test_input)
print(f"\nDecorated result: {repr(result_decorated)}")
print(f"Same result: {result == result_decorated}")
