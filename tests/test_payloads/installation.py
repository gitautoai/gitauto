from config import (
    TEST_APP_ID,
    TEST_INSTALLATION_ID,
    TEST_NEW_INSTALLATION_ID,
    TEST_OWNER_ID,
    TEST_OWNER_NAME,
    TEST_OWNER_TYPE,
    TEST_REPO_ID,
    TEST_REPO_NAME,
    TEST_USER_ID,
    TEST_USER_NAME,
)
from services.github.github_types import GitHubEventPayload


installation_payload: GitHubEventPayload = {
    "action": "created",
    "installation": {
        "id": TEST_INSTALLATION_ID,
        "account": {
            "login": TEST_OWNER_NAME,
            "id": TEST_OWNER_ID,
            "node_id": "O_kgDOB9UpXA",
            "avatar_url": "https://avatars.githubusercontent.com/u/-1?v=4",
            "gravatar_id": "",
            "url": "https://api.github.com/users/installation-test",
            "html_url": "https://github.com/installation-test",
            "followers_url": "https://api.github.com/users/installation-test/followers",
            "following_url": "https://api.github.com/users/installation-test/following{/other_user}",
            "gists_url": "https://api.github.com/users/installation-test/gists{/gist_id}",
            "starred_url": "https://api.github.com/users/installation-test/starred{/owner}{/repo}",
            "subscriptions_url": "https://api.github.com/users/installation-test/subscriptions",
            "organizations_url": "https://api.github.com/users/installation-test/orgs",
            "repos_url": "https://api.github.com/users/installation-test/repos",
            "events_url": "https://api.github.com/users/installation-test/events{/privacy}",
            "received_events_url": "https://api.github.com/users/installation-test/received_events",
            "type": TEST_OWNER_TYPE,
            "site_admin": False,
        },
        "repository_selection": "all",
        "access_tokens_url": f"https://api.github.com/app/installations/{TEST_INSTALLATION_ID}/access_tokens",
        "repositories_url": "https://api.github.com/installation/repositories",
        "html_url": f"https://github.com/organizations/{TEST_OWNER_NAME}/settings/installations/{TEST_INSTALLATION_ID}",
        "app_id": TEST_APP_ID,
        "app_slug": "issue-to-pull-request",
        "target_id": TEST_INSTALLATION_ID,
        "target_type": TEST_OWNER_TYPE,
        "permissions": {
            "contents": "write",
            "issues": "write",
            "metadata": "read",
            "pull_requests": "write",
            "workflows": "write",
        },
        "events": [
            "commit_comment",
            "issues",
            "issue_comment",
            "label",
            "public",
            "pull_request",
            "pull_request_review",
            "pull_request_review_comment",
            "pull_request_review_thread",
        ],
        "created_at": "2024-04-19T23:52:28.000-07:00",
        "updated_at": "2024-04-19T23:52:29.000-07:00",
        "single_file_name": None,
        "has_multiple_single_files": False,
        "single_file_paths": [],
        "suspended_by": None,
        "suspended_at": None,
    },
    "repositories": [
        {
            "id": TEST_REPO_ID,
            "node_id": "R_kgDOJZc8Zg",
            "name": TEST_REPO_NAME,
            "full_name": f"{TEST_OWNER_NAME}/{TEST_REPO_NAME}",
            "private": True,
        }
    ],
    "requester": None,
    "sender": {
        "login": TEST_USER_NAME,
        "id": TEST_USER_ID,
        "node_id": "MDQ6VXNlcjY2Njk5Mjkw",
        "avatar_url": "https://avatars.githubusercontent.com/u/-1?v=4",
        "gravatar_id": "",
        "url": "https://api.github.com/users/username-test",
        "html_url": "https://github.com/username-test",
        "followers_url": "https://api.github.com/users/username-test/followers",
        "following_url": "https://api.github.com/users/username-test/following{/other_user}",
        "gists_url": "https://api.github.com/users/username-test/gists{/gist_id}",
        "starred_url": "https://api.github.com/users/username-test/starred{/owner}{/repo}",
        "subscriptions_url": "https://api.github.com/users/username-test/subscriptions",
        "organizations_url": "https://api.github.com/users/username-test/orgs",
        "repos_url": "https://api.github.com/users/username-test/repos",
        "events_url": "https://api.github.com/users/username-test/events{/privacy}",
        "received_events_url": "https://api.github.com/users/username-test/received_events",
        "type": "User",
        "site_admin": False,
    },
}


new_installation_payload: GitHubEventPayload = {
    "action": "created",
    "installation": {
        "id": TEST_NEW_INSTALLATION_ID,
        "account": {
            "login": TEST_OWNER_NAME,
            "id": TEST_OWNER_ID,
            "node_id": "O_kgDOB9UpXA",
            "avatar_url": "https://avatars.githubusercontent.com/u/-1?v=4",
            "gravatar_id": "",
            "url": "https://api.github.com/users/installation-test",
            "html_url": "https://github.com/installation-test",
            "followers_url": "https://api.github.com/users/installation-test/followers",
            "following_url": "https://api.github.com/users/installation-test/following{/other_user}",
            "gists_url": "https://api.github.com/users/installation-test/gists{/gist_id}",
            "starred_url": "https://api.github.com/users/installation-test/starred{/owner}{/repo}",
            "subscriptions_url": "https://api.github.com/users/installation-test/subscriptions",
            "organizations_url": "https://api.github.com/users/installation-test/orgs",
            "repos_url": "https://api.github.com/users/installation-test/repos",
            "events_url": "https://api.github.com/users/installation-test/events{/privacy}",
            "received_events_url": "https://api.github.com/users/installation-test/received_events",
            "type": TEST_OWNER_TYPE,
            "site_admin": False,
        },
        "repository_selection": "all",
        "access_tokens_url": f"https://api.github.com/app/installations/{TEST_NEW_INSTALLATION_ID}/access_tokens",
        "repositories_url": "https://api.github.com/installation/repositories",
        "html_url": f"https://github.com/organizations/{TEST_OWNER_NAME}/settings/installations/{TEST_NEW_INSTALLATION_ID}",
        "app_id": TEST_APP_ID,
        "app_slug": "issue-to-pull-request",
        "target_id": TEST_NEW_INSTALLATION_ID,
        "target_type": TEST_OWNER_TYPE,
        "permissions": {
            "contents": "write",
            "issues": "write",
            "metadata": "read",
            "pull_requests": "write",
            "workflows": "write",
        },
        "events": [
            "commit_comment",
            "issues",
            "issue_comment",
            "label",
            "public",
            "pull_request",
            "pull_request_review",
            "pull_request_review_comment",
            "pull_request_review_thread",
        ],
        "created_at": "2024-04-19T23:52:28.000-07:00",
        "updated_at": "2024-04-19T23:52:29.000-07:00",
        "single_file_name": None,
        "has_multiple_single_files": False,
        "single_file_paths": [],
        "suspended_by": None,
        "suspended_at": None,
    },
    "repositories": [
        {
            "id": TEST_REPO_ID,
            "node_id": "R_kgDOJZc8Zg",
            "name": TEST_REPO_NAME,
            "full_name": f"{TEST_OWNER_NAME}/{TEST_REPO_NAME}",
            "private": True,
        }
    ],
    "requester": None,
    "sender": {
        "login": TEST_USER_NAME,
        "id": TEST_USER_ID,
        "node_id": "MDQ6VXNlcjY2Njk5Mjkw",
        "avatar_url": "https://avatars.githubusercontent.com/u/-1?v=4",
        "gravatar_id": "",
        "url": "https://api.github.com/users/username-test",
        "html_url": "https://github.com/username-test",
        "followers_url": "https://api.github.com/users/username-test/followers",
        "following_url": "https://api.github.com/users/username-test/following{/other_user}",
        "gists_url": "https://api.github.com/users/username-test/gists{/gist_id}",
        "starred_url": "https://api.github.com/users/username-test/starred{/owner}{/repo}",
        "subscriptions_url": "https://api.github.com/users/username-test/subscriptions",
        "organizations_url": "https://api.github.com/users/username-test/orgs",
        "repos_url": "https://api.github.com/users/username-test/repos",
        "events_url": "https://api.github.com/users/username-test/events{/privacy}",
        "received_events_url": "https://api.github.com/users/username-test/received_events",
        "type": "User",
        "site_admin": False,
    },
}
