...
import pytest
from services.github.pulls.get_pull_request_file_contents import get_pull_request_file_contents
...
def mock_create_headers():
    with patch("services.github.pulls.get_pull_request_file_contents.create_headers") as mock:
...
def mock_get_remote_file_content():
    with patch("services.github.pulls.get_pull_request_file_contents.get_remote_file_content") as mock:
        yield mock


def test_get_pull_request_file_contents_success(mock_requests, mock_create_headers, mock_get_remote_file_content):
    # Arrange
    ...
    mock_get_remote_file_content.assert_any_call(file_path="file1.py", base_args=base_args)
    mock_get_remote_file_content.assert_any_call(file_path="file2.py", base_args=base_args)


def test_get_pull_request_file_contents_empty(mock_requests, mock_create_headers, mock_get_remote_file_content):
    # Arrange
    url = "https://api.github.com/repos/owner/repo/pulls/1/files"
    base_args = {"token": "test_token"}
