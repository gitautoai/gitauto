...
def filter_code_files(filenames: list[str]):
    ...
    for filename in filenames:
        ...
        if any(basename == p + ".py" or basename.startswith(p + "_") or basename.endswith("_" + p + ".py") 
              for p in ["mock", "stub", "fixture"]):
            continue
            
        # Special case for files that contain test-related words but are not test files
