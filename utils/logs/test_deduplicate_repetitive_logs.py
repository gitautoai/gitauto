...
def test_sentry_issue_agent_146_real_error_log():
    ...
    deduplicated = deduplicate_repetitive_logs(agent_146_log)
    ...
    # Exact size assertions for predictable behavior
    assert (
        original_length == 194117
    ), f"Expected exactly 194117 chars in original log, got {original_length:,}"
    assert (
        deduplicated_length == 61507
    ), f"Expected exactly 61507 chars after deduplication, got {deduplicated_length:,}"
    assert (
        abs(reduction_percent - 68.3) < 0.1
    ), f"Expected exactly 68.3% reduction, got {reduction_percent:.1f}%"
    ...
    assert (
        deduplicated_length == 61507
    ), f"Deduplicated log should be exactly 61507 chars, got {deduplicated_length:,}"
...
"""
    ...
    deduplicated = deduplicate_repetitive_logs(insurance_json_log)
...
if __name__ == "__main__":
    ...
    print("All Sentry issue AGENT-146 deduplication tests passed!")
